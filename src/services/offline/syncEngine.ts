...
/**
 * Carga el intent activo actual en la tienda (utilizado al inicializar o montar componentes).
 */
export const loadActiveOutboxIntent = async (): Promise<OutboxIntentRow | null> => {
  const active = await outboxRepository.getActiveIntent();
  useOutboxStore.setState({ intent: active });
  return active;
};
...
/**
 * Obtiene el estado actual de la bandeja de salida (para consultas desde hooks/UI).
 */
export const getOutboxStatus = async (): Promise<OutboxIntentRow | null> => {
  return outboxRepository.getActiveIntent();
};

/**
 * Obtiene el intent activo actual.
 */
...
/**
 * Resuelve un conflicto sobrescribiendo con los cambios locales del usuario (manteniendo los suyos):
 * vuelve a poner el intent en estado 'pending' actualizando su timestamp de expectativa.
 */
export const resolveConflictKeepMine = async (expectedProfileUpdatedAt: string, queryClient: QueryClient): Promise<void> => {
  const active = await outboxRepository.getActiveIntent();
  if (!active) return;
...
/**
 * Forzar un reintento manual inmediato del outbox actual.
 */
