...
import type { ThemeMode, DisplayDensity } from '../services/storage/userPreferences';
...
describe('store/preferencesStore', () => {
  ...
  it('updatePreference updates single key, persists to storage and applies theme', async () => {
    ...
    await usePreferencesStore.getState().updatePreference('theme', 'dark' as ThemeMode);
  ...
  it('resetPreferences clears storage, applies defaults and notifies', async () => {
    ...
    expect(userPreferencesService.applyDisplayDensity).toHaveBeenCalledWith('comfortable');
  });
  ...
  it('sets error when saveUserPreferences fails', async () => {
    ...
    await usePreferencesStore.getState().updatePreference('theme', 'light' as ThemeMode);
