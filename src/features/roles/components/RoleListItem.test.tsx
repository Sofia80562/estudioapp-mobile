...
const ROLE: RoleDto = {
  ...
  code: 'gestor-de-tareas',
  description: 'Organiza recordatorios y tareas académicas.',
  isSystem: false,
  permissionsCount: 2,
...
describe('RoleListItem', () => {
  it('abre la edición conservando la organización seleccionada', () => {
    ...
    fireEvent.click(screen.getByLabelText('Editar Gestor de Tareas'));
  ...
  it('no ofrece edición para roles del sistema', () => {
    ...
    expect(screen.queryByLabelText('Editar Gestor de Tareas')).not.toBeInTheDocument();
  });
});