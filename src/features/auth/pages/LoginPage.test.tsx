...
describe('LoginPage', () => {
  it('shows the native credential form with accessible labels', () => {
    ...
    expect(container.querySelector('ion-input[label="Correo o Usuario"]')).toBeInTheDocument();
  ...
  it('identifies required fields when the native form is submitted empty', async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector('ion-input[label="Correo o Usuario"]')).toHaveAttribute(
        'error-text',
  ...
  it('keeps the web login free of credential fields', () => {
    ...
    expect(container.querySelector('ion-input[label="Correo o Usuario"]')).not.toBeInTheDocument();
