...
const OwnProfilePage: React.FC = () => {
  const user = useSessionStore((state: { user: { name: string } | null }) => state.user);
  ...
  const upload = async (body: Parameters<typeof updateAvatar.mutateAsync>[0]): Promise<void> => {
    ...
    try {
      ...
      const err = caught as AppClientError;
      throw new Error(err instanceof AppClientError ? err.message : 'No se pudo actualizar la fotografía.');
    }
  };
