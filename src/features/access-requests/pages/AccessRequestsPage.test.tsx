...
const PENDING_REQUEST = {
  ...
  organization: {
    ...
    venues: [{ id: 'venue-1', name: 'Campus Principal', status: 'PENDING_APPROVAL' }],
  },
  requester: {
    id: 'user-1',
...
describe('AccessRequestsPage', () => {
  ...
  it('asks for confirmation before approving, and calls the endpoint only after confirming', async () => {
    ...
    expect(await screen.findByRole('status', { name: 'Solicitud aprobada' })).toHaveTextContent(
  ...
  it('asks for confirmation before rejecting, and calls the endpoint only after confirming', async () => {
    ...
    expect(await screen.findByRole('status', { name: 'Solicitud rechazada' })).toHaveTextContent(
      'Institución Educativa Central',
    );
  });
